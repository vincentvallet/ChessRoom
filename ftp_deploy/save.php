...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $jsonData = @json_decode($data, true);
        if ($jsonData) {
            $regCount = isset($jsonData['registrations']) ? count($jsonData['registrations']) : 0;
            @file_put_contents($debugLog, date('Y-m-d H:i:s') . " SAVE: Registrations count = $regCount\n", FILE_APPEND);
        }
        ...
        if (@flock($fp, LOCK_EX)) {
            ...
            $verifyData = @file_get_contents($dataFile);
            $verifyJson = @json_decode($verifyData, true);
            $verifyRegCount = isset($verifyJson['registrations']) ? count($verifyJson['registrations']) : 0;
            @file_put_contents($debugLog, date('Y-m-d H:i:s') . " VERIFY: File written, registrations = $verifyRegCount\n", FILE_APPEND);
